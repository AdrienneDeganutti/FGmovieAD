...
class MADdataset(torch.utils.data.Dataset):
    ...
    def __init__(self, annotations_file, root, visual_feats_file, tokenizer_folder, num_pre_clips,
                num_clips, pre_query_size, split, fps, test_stride, input_stride, 
                neg_prob, language_feats_file, lang_feat_type):
        ...
        if os.path.exists(cache):
            ...
            self._compute_annotations(annos, cache)

        # Get correct data for language
        if lang_feat_type == 'clip' and os.path.exists(language_feats_file):
            self.load_clip_lang_feats(language_feats_file)
        else:
    ...
    def _compute_annotations(self, annos, cache):
        ...
        # save to file
        pkl.dump(self.annos,open(cache,'wb'))
        logger.info("Successfully saved annotations to cache.")
